...
//	$user = ($user)?$user:header("location:../application/pages/login.php");
	...
	foreach($progs as $prog){
		...
		$program_results .= template_parse("pdf_templates_test/programs.tpl", $progs_replace);
		$prog_count++;
	}
	...
	foreach($institutions as $institution){
		...
		$institution_results .= template_parse("pdf_templates_test/previous_institutions.tpl", $institution_replace);
	}
	...
	foreach($dviolations as $dviolation){
		...
		$dviolation_results .= template_parse("pdf_templates_test/disciplinary_violations.tpl", $dviolation_replace);
	}
	...
	foreach($cviolations as $cviolation){
		...
		$cviolation_results .= template_parse("pdf_templates_test/criminal_violations.tpl", $cviolation_replace);
	}
	...
	foreach($gres as $gre){
		...
		$gre_results .= template_parse("pdf_templates_test/gre_scores.tpl", $gre_replace);
	}
	...
	foreach($languages as $language){
		...
		$language_results .= template_parse("pdf_templates_test/languages.tpl", $language_replace);
	}
	...
	foreach($recommendations as $recommendation){
		...
		//substitute
		$recommendation_results .= template_parse("pdf_templates_test/extra_recommendations.tpl", $recommendation_replace);
	...
	*/
	...
	$all_html_content = "";
	$all_html_content .= template_parse("pdf_templates_test/app_page_1.tpl", $page1_replace);	
	$all_html_content .= template_parse("pdf_templates_test/app_page_2.tpl", $page2_replace);
	$all_html_content .= template_parse("pdf_templates_test/app_page_3.tpl", $page3_replace);
	$all_html_content .= template_parse("pdf_templates_test/app_page_4.tpl", $page4_replace);
	...
	//TEST//
	$all_html_content = template_parse("pdf_templates_test/Application.html", $dviolation_replace);
	//MPDF	
	include('MPDF52/mpdf.php');
	$mpdf = new mPDF();
	$mpdf->WriteHTML($all_html_content);
	$mpdf->Output("completed_pdfs_trial/".$pdftitle);

//	$pdf->Output("completed_pdfs_trial/".$pdftitle, 'F');

	//change permissions
	$cwd = "completed_pdfs_trial/";
	$cwd .= $pdftitle;
	chmod($cwd, 0664);
