<?php
	...
	include_once "../application/templates/template.php";
	...
	$full_name = $personal_data[1] ."_". $personal_data[2] ."_". $personal_data[3];
	...
	$institution_data = $db->query($primary_query, $user);
	...
	$institution_results = "";
	$institution_query = "";
	$institution_query .= "SELECT * FROM previousschools WHERE applicant_id=%d";
	$institutions = $db->query($institution_query, $user);
	...
	$dviolation_results = "";
	$dviolation_query = "";
	$dviolation_query .= "SELECT * FROM dviolations WHERE applicant_id=%d";
	$dviolations = $db->query($dviolation_query, $user);
	...
	$cviolation_results = "";
	$cviolation_query = "";
	$cviolation_query .= "SELECT * FROM cviolations WHERE applicant_id=%d";
	$cviolations = $db->query($cviolation_query, $user);
	...
	$page3_data = $db->query($primary_query, $user);
	...
	$gre_results = "";
	$gre_query = "";
	$gre_query .= "SELECT * FROM gre WHERE applicant_id=%d";
	$gres = $db->query($gre_query, $user);
	foreach($gres as $gre){
		$gre_replace = array();
	...
	$language_results = "";
	$language_query = "";
	$language_query .= "SELECT * FROM languages WHERE applicant_id=%d";
	$languages = $db->query($language_query, $user);
	...
	$recommendation_query .= "SELECT * FROM extrareferences WHERE applicant_id=%d";
	$recommendations = $db->query($recommendation_query, $user);
	...
	$primary_query = "";
	$primary_query .= "SELECT * FROM `international` WHERE `applicant_id` = %d";
	
	$page4_data = $db->query($primary_query, $user);
...
//	$today .= date("U");//append unix timestamp for server copy of PDF
	$pdftitle = "UMaineGradSchool_". $today ."_". $full_name .".pdf";
