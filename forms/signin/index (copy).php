...
if ($_GET) {
	if (isset($_GET['email']) && isset($_GET['code'])) {
		...
		if ($check_user != null) {
			...
			//echo "check_id is $check_id and check_login_email_confirmed is $check_login_email_confirmed";
			...
			if ($check_login_email_confirmed == 1) {
				...
				$success .= "<p class='warning'>You have been confirmed. Please sign in.</p>";
		...
		if ($email == 'e-mail address') {
			...
			if ($user_result != null) {
				...
				if ($hash == sha1($password) && $confirmed == 1) {
					user_login($id);
					if (check_ses_vars()) {
						header('Location: ../../application/app_manager.php');
					}
...
<head>
	...
	<script type="text/javascript" src="includes/sha1-min.js" charset="utf-8"></script>
	<script type="text/javascript" src="includes/browser_check.js" charset="utf-8"></script>
	...
	<!-- JQUERY -->
	<link type="text/css" href="../../application/styles/jquery/redmond/jquery-ui-1.8.2.custom.css" rel="Stylesheet" />
	<script type="text/javascript" src="../../application/libs/jquery/jquery-1.4.2.min.js"></script>
	<script type="text/javascript" src="../../application/libs/jquery/jquery-ui-1.8.2.custom.min.js"></script>
	...
	html {
		background-image:url(../../drupal6/sites/all/themes/acquia_marina/images/background-tile2.png);
	}
	...
	.gradHeader {
		...
		height:85px;
		margin-top:10px;
		background-image:url('../../drupal6/sites/default/files/acquia_marina_logo.png');
...
<body onLoad="doBrowserCheck();">
	...
	<!--	<div class="gradHeader"></div><br/> -->
		...
		<div id="signin">
			<div di="success_msg"><?php print $signin_msg.$success_msg; ?></div>
			<form id="signinForm" method="post" accept-charset="utf-8">
				...
				<p class="message"><strong>Note:</strong> if you are not applying for a degree, please use the <a href="http://hoonah.asap.um.maine.edu/grad/drupal6/documents/file/Nondegree.pdf">graduate nondegree application/registration form</a>.</p>
				<p class="message"><strong>Note:</strong> if you already started filling out an application on our <a href="http://www2.umaine.edu/graduate/article.php?id=63">old online application</a>, please complete your application there.</p>
			</form>
		</div>
