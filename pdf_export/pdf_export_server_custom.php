<?php
	...
	//*********************************************************************************************
	$user = $_GET['user'];
	$results = $db->query("SELECT applicant_id FROM applicants WHERE applicant_id = %d", $user);
	if($results == Array()) {
		echo "No user with id " . $_GET['user'];
	} else {
	...
	foreach($recommendations as $recommendation){
		...
		foreach($recommendation as $id_key => $value){
			if(!is_numeric($id_key)) {
				$recommendation_replace[strtoupper($id_key)] = $value;
			}
		}
		//replace code
...
//		}
//		else{
//					header("location:application/pages/login.php");
