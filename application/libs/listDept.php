<?php
	...
	$results = "";
	if($_POST['type'] == "dept") {
		$programs = $db->query("SELECT DISTINCT academic_dept_heading, academic_dept_code FROM um_academic ORDER BY academic_dept ASC");
		...
		if($_POST['dept']){
			$programs = $db->query("SELECT academic_program, academic_dept_code, academic_degree_heading FROM um_academic WHERE academic_dept_code='%s'", $_POST['dept']);
			foreach($programs as $program){
			...
			$programs = $db->query("SELECT academic_degree_heading,academic_dept_heading,academic_program FROM um_academic ORDER BY description_app ASC");
			foreach($programs as $program){
