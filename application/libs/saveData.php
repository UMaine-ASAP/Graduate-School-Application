...
function filter_ssn($value) {
	if(strlen($value) == 11
		&& $value[3] == "-"
		&& $value[6] == "-"
		&& filter_var(substr($value, 0, 3), FILTER_VALIDATE_INT)
		&& (int)substr($value, 0, 3)!=0
		&& !((int)substr($value, 0, 3)>899)
		&& !((int)substr($value, 0, 3)==666)
		&& filter_var(substr($value, 4, 2), FILTER_VALIDATE_INT)
		&& (int)substr($value, 4, 2)!=0
		&& filter_var(substr($value, 7, 4), FILTER_VALIDATE_INT)
...
function filter_state($value) {
	$value = trim($value);
	if(preg_match("(INT|AL|AK|AZ|AR|CA|CO|CT|DC|DE|FL|GA|HI|ID|IL|IN|IA|KS|KY|LA|ME| MD|MA|MI|MN|MS|MO|MT|NE|NV|NH|NJ|NM|NY|NC|ND|OH|OK|OR|PA|RI|SC|SD|TN|TX|UT|VT|VA|WA|WV|WI|WY|AB|BC|MB|NB|NL|NS|ON|PE|QC|SK|NT|NU|YT)", $value))
		return $value;
	else
		return false;
}
